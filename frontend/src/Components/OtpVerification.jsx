...
const OtpVerification = () => {
    ...
    return (
        <div className="flex flex-col justify-center items-center h-screen">
            <Toaster />
            <h1 className="text-2xl font-semibold text-black mb-4">OTP Verification</h1>
            <div className="space-x-2">
                {otp.map((data, index) => (
                    <input
                        className="w-12 h-12 text-xl text-center border-2 border-black focus:border-green-700 rounded"
                        key={index}
                        type="text"
                        maxLength="1"
                        value={data}
                ...
                <button
                    className="mt-4 bg-black text-white font-semibold py-2 px-4 rounded"
                    onClick={submitOtp}
                >
                    Verify OTP
                </button>
