...
const ReportPending = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (data) {
        const filtered = data?.data?.filter((item)=>item?.isReported === false)
  ...
  const sendReport = async (predId) => {
    try{
        const userId = localStorage.getItem("id")
        const response = await axios.get(`${process.env.REACT_APP_BASE_URI}/report/${predId}/${userId}`)
  ...
  const columns = [
    ...
    },
    {
        ...
        key: 'report',
        render: (text, record) => (
          <button className='bg-black px-4 py-2 rounded-md text-white' onClick={() => sendReport(record.id)}>Report</button>
        )
      }
